/**
 * @typedef {'Head'|'Body'|'Shield'|'Accessory'} ArmorType
 */

/**
 * @typedef {'C'|'E'|'L'|'H'|'V'|'R'|'S'|'M'|'F'|'K'|'N'} WearClass
 */

/**
 * @typedef {Object} ArmorSources
 * @property {string[]} shop
 * @property {string[]} treasure
 * @property {string[]} loot
 */

/**
 * @typedef {Object} ArmorPiece
 * @property {string} name
 * @property {ArmorType} type
 * @property {number} bits
 * @property {number} def
 * @property {WearClass[]} wear
 * @property {string[]} effects
 * @property {ArmorSources} sources
 */

export const ARMOR_TYPES = {
  HEAD: /** @type {ArmorType} */ ('Head'),
  BODY: /** @type {ArmorType} */ ('Body'),
  SHIELD: /** @type {ArmorType} */ ('Shield'),
  ACCESSORY: /** @type {ArmorType} */ ('Accessory'),
};

export const WEAR_CLASSES = {
  CAP: /** @type {WearClass} */ ('C'),
  HELMET: /** @type {WearClass} */ ('E'),
  LIGHT_ARMOR: /** @type {WearClass} */ ('L'),
  HEAVY_ARMOR: /** @type {WearClass} */ ('H'),
  VEST: /** @type {WearClass} */ ('V'),
  ROBE: /** @type {WearClass} */ ('R'),
  SHIELD: /** @type {WearClass} */ ('S'),
  MALE: /** @type {WearClass} */ ('M'),
  FEMALE: /** @type {WearClass} */ ('F'),
  KOBOLD: /** @type {WearClass} */ ('K'),
  NOBILITY: /** @type {WearClass} */ ('N'),
};

/** @type {Record<string, ArmorPiece>} */
export const ARMOR = {
  BANDANNA: {
    name: 'Bandanna',
    type: ARMOR_TYPES.HEAD,
    bits: 50,
    def: 1,
    wear: [WEAR_CLASSES.CAP],
    effects: [],
    sources: {
      shop: ['Gregminster #1'],
      treasure: [],
      loot: ['Crow'],
    },
  },
  HEADBAND: {
    name: 'Headband',
    type: ARMOR_TYPES.HEAD,
    bits: 300,
    def: 2,
    wear: [WEAR_CLASSES.CAP, WEAR_CLASSES.HELMET],
    effects: [],
    sources: {
      shop: ['Gregminster', 'Rockland', 'Lenankamp'],
      treasure: [],
      loot: [],
    },
  },
  POINTED_HAT: {
    name: 'Pointed Hat',
    type: ARMOR_TYPES.HEAD,
    bits: 1200,
    def: 5,
    wear: [WEAR_CLASSES.CAP],
    effects: ['Prevents "Balloon"'],
    sources: {
      shop: ['Lenankamp', 'Kaku'],
      treasure: ['Toran Basement', "Dwarves' Vault"],
      loot: ['Red Soldier Ant', 'Soldier Ant', 'Robot Soldier (spear)'],
    },
  },
  HALF_HELMET: {
    name: 'Half Helmet',
    type: ARMOR_TYPES.HEAD,
    bits: 3300,
    def: 9,
    wear: [WEAR_CLASSES.CAP, WEAR_CLASSES.HELMET],
    effects: [],
    sources: {
      shop: ['Dwarf Village', 'Antei', 'Kirov'],
      treasure: ['Soniere'],
      loot: ['Death Machine (spear)'],
    },
  },
  HEAD_GEAR: {
    name: 'Head Gear',
    type: ARMOR_TYPES.HEAD,
    bits: 6500,
    def: 14,
    wear: [WEAR_CLASSES.CAP],
    effects: [],
    sources: {
      shop: ['Antei', "Warrior's Village"],
      treasure: ['Qlon Cave'],
      loot: ['Mad Ivy', 'Grizzly Bear'],
    },
  },
  FULL_HELMET: {
    name: 'Full Helmet',
    type: ARMOR_TYPES.HEAD,
    bits: 13200,
    def: 20,
    wear: [WEAR_CLASSES.HELMET],
    effects: [],
    sources: {
      shop: ["Warrior's Village"],
      treasure: ["Neclord's Castle"],
      loot: ['Grave Master'],
    },
  },
  SILVER_HAT: {
    name: 'Silver Hat',
    type: ARMOR_TYPES.HEAD,
    bits: 27000,
    def: 27,
    wear: [WEAR_CLASSES.CAP],
    effects: ['Auto-heal +5'],
    sources: {
      shop: ['Gregminster #2'],
      treasure: [],
      loot: ['Queen Ant'],
    },
  },
  HORNED_HELMET: {
    name: 'Horned Helmet',
    type: ARMOR_TYPES.HEAD,
    bits: 43000,
    def: 35,
    wear: [WEAR_CLASSES.HELMET],
    effects: [],
    sources: {
      shop: [],
      treasure: ['Gregminster Palace'],
      loot: ['Elite Soldier'],
    },
  },
  ROBE: {
    name: 'Robe',
    type: ARMOR_TYPES.BODY,
    bits: 100,
    def: 1,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.VEST, WEAR_CLASSES.ROBE],
    effects: [],
    sources: {
      shop: ['Gregminster #1'],
      treasure: [],
      loot: ['Holly Boy'],
    },
  },
  TUNIC: {
    name: 'Tunic',
    type: ARMOR_TYPES.BODY,
    bits: 200,
    def: 2,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.VEST],
    effects: [],
    sources: {
      shop: ['Gregminster #1', 'Rockland', 'Lenankamp'],
      treasure: [],
      loot: ['Bandit (green)'],
    },
  },
  LEATHER_COAT: {
    name: 'Leather Coat',
    type: ARMOR_TYPES.BODY,
    bits: 700,
    def: 4,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.HEAVY_ARMOR, WEAR_CLASSES.VEST],
    effects: [],
    sources: {
      shop: ['Gregminster #1', 'Rockland', 'Lenankamp', 'Kaku'],
      treasure: ["Magician's Island"],
      loot: [],
    },
  },
  BRASS_ARMOR: {
    name: 'Brass Armor',
    type: ARMOR_TYPES.BODY,
    bits: 1000,
    def: 5,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.HEAVY_ARMOR],
    effects: [],
    sources: {
      shop: ['Lenankamp', 'Kaku', 'Dwarf Village'],
      treasure: ["Grady's Mansion"],
      loot: ['Ghost Armor', 'Empire Soldier (sword #4)'],
    },
  },
  GUARD_ROBE: {
    name: 'Guard Robe',
    type: ARMOR_TYPES.BODY,
    bits: 1700,
    def: 7,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.VEST, WEAR_CLASSES.ROBE],
    effects: ['MAG +15'],
    sources: {
      shop: ['Kaku'],
      treasure: ["Dwarves' Trail"],
      loot: ['Kobold (mage)'],
    },
  },
  KARATE_UNIFORM: {
    name: 'Karate Uniform',
    type: ARMOR_TYPES.BODY,
    bits: 3000,
    def: 10,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.VEST],
    effects: [],
    sources: {
      shop: ['Kouan'],
      treasure: ["Dwarves' Trail"],
      loot: ['Slasher Rabbit', 'Dwarf'],
    },
  },
  LEATHER_ARMOR: {
    name: 'Leather Armor',
    type: ARMOR_TYPES.BODY,
    bits: 5900,
    def: 14,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.HEAVY_ARMOR],
    effects: [],
    sources: {
      shop: ['Kouan', 'Dwarf Village'],
      treasure: ["Lepant's House"],
      loot: ['Veteran Soldier (spear)'],
    },
  },
  HALF_ARMOR: {
    name: 'Half Armor',
    type: ARMOR_TYPES.BODY,
    bits: 8700,
    def: 18,
    wear: [WEAR_CLASSES.HEAVY_ARMOR],
    effects: [],
    sources: {
      shop: ['Dwarf Village'],
      treasure: ['Panna Yakuta'],
      loot: ['Dragon'],
    },
  },
  MAGIC_ROBE: {
    name: 'Magic Robe',
    type: ARMOR_TYPES.BODY,
    bits: 15000,
    def: 22,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.VEST, WEAR_CLASSES.ROBE],
    effects: [],
    sources: {
      shop: ['Antei'],
      treasure: ["Dwarves' Vault", 'Scarleticia'],
      loot: ['Holly Fairy'],
    },
  },
  NINJA_SUIT: {
    name: 'Ninja Suit',
    type: ARMOR_TYPES.BODY,
    bits: 22000,
    def: 28,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.VEST],
    effects: ['SPD +5'],
    sources: {
      shop: ['Antei', 'Kirov'],
      treasure: [],
      loot: ['Viperman'],
    },
  },
  DRAGON_ARMOR: {
    name: 'Dragon Armor',
    type: ARMOR_TYPES.BODY,
    bits: 37000,
    def: 34,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.HEAVY_ARMOR],
    effects: [],
    sources: {
      shop: ["Warrior's Village", 'Kirov', 'Gregminster #2'],
      treasure: [],
      loot: ['Dagon'],
    },
  },
  MASTER_ROBE: {
    name: 'Master Robe',
    type: ARMOR_TYPES.BODY,
    bits: 78000,
    def: 37,
    wear: [WEAR_CLASSES.VEST, WEAR_CLASSES.ROBE],
    effects: ['Auto-heal +5'],
    sources: {
      shop: ['Gregminster #2'],
      treasure: ['Seek Valley'],
      loot: ['Sorcerer'],
    },
  },
  FULL_ARMOR: {
    name: 'Full Armor',
    type: ARMOR_TYPES.BODY,
    bits: 57000,
    def: 45,
    wear: [WEAR_CLASSES.HEAVY_ARMOR],
    effects: [],
    sources: {
      shop: ["Warrior's Village"],
      treasure: [],
      loot: ['Clay Doll'],
    },
  },
  TAIKIOKU_WEAR: {
    name: 'Taikioku Wear',
    type: ARMOR_TYPES.BODY,
    bits: 80000,
    def: 48,
    wear: [WEAR_CLASSES.VEST],
    effects: ['SPD +10'],
    sources: {
      shop: [],
      treasure: ['Moravia'],
      loot: ['Rock Buster'],
    },
  },
  MASTER_GARB: {
    name: 'Master Garb',
    type: ARMOR_TYPES.BODY,
    bits: 93000,
    def: 55,
    wear: [WEAR_CLASSES.LIGHT_ARMOR, WEAR_CLASSES.HEAVY_ARMOR],
    effects: ['SKL +5'],
    sources: {
      shop: [],
      treasure: ['Shasarazade'],
      loot: ['Earth Golem'],
    },
  },
  WINDSPUN_ARMOR: {
    name: 'Windspun Armor',
    type: ARMOR_TYPES.BODY,
    bits: 120000,
    def: 63,
    wear: [WEAR_CLASSES.HEAVY_ARMOR],
    effects: ['SPD +20'],
    sources: {
      shop: [],
      treasure: ['Gregminster Palace'],
      loot: ['Ekidonna'],
    },
  },
  WOODEN_SHIELD: {
    name: 'Wooden Shield',
    type: ARMOR_TYPES.SHIELD,
    bits: 300,
    def: 2,
    wear: [WEAR_CLASSES.SHIELD],
    effects: [],
    sources: {
      shop: ['Rockland', 'Kaku'],
      treasure: [],
      loot: ['Kobold (sword)'],
    },
  },
  STEEL_SHIELD: {
    name: 'Steel Shield',
    type: ARMOR_TYPES.SHIELD,
    bits: 7300,
    def: 13,
    wear: [WEAR_CLASSES.SHIELD],
    effects: [],
    sources: {
      shop: ['Kouan'],
      treasure: ['Panna Yakuta'],
      loot: ['Death Machine (sword)'],
    },
  },
  CHAOS_SHIELD: {
    name: 'Chaos Shield',
    type: ARMOR_TYPES.SHIELD,
    bits: 32000,
    def: 27,
    wear: [WEAR_CLASSES.SHIELD],
    effects: [],
    sources: {
      shop: ['Kirov'],
      treasure: ['Shasarazade'],
      loot: ['Devil Shield'],
    },
  },
  EARTH_SHIELD: {
    name: 'Earth Shield',
    type: ARMOR_TYPES.SHIELD,
    bits: 68000,
    def: 45,
    wear: [WEAR_CLASSES.SHIELD],
    effects: [],
    sources: {
      shop: ['Gregminster #2'],
      treasure: [],
      loot: ['Magic Shield'],
    },
  },
  WOODEN_SHOES: {
    name: 'Wooden Shoes',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 100,
    def: 1,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Gregminster #1'],
      treasure: [],
      loot: ['Fur Fur'],
    },
  },
  LEGGINGS: {
    name: 'Leggings',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 200,
    def: 2,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Rockland'],
      treasure: ['Mt. Seifu'],
      loot: [],
    },
  },
  GLOVES: {
    name: 'Gloves',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 300,
    def: 2,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Rockland', 'Kaku'],
      treasure: ['Mt. Tigerwolf', 'Panna Yakuta'],
      loot: [],
    },
  },
  CAPE: {
    name: 'Cape',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 400,
    def: 2,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Lenankamp', 'Kaku'],
      treasure: [],
      loot: [],
    },
  },
  CIRCLET: {
    name: 'Circlet',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 600,
    def: 3,
    wear: [WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Lenankamp'],
      treasure: ['Mt. Tigerwolf'],
      loot: [],
    },
  },
  BOOTS: {
    name: 'Boots',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 800,
    def: 3,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Kouan'],
      treasure: ['Mt. Seifu'],
      loot: [],
    },
  },
  GAUNTLET: {
    name: 'Gauntlet',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 1700,
    def: 4,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Kouan'],
      treasure: ['Great Forest'],
      loot: [],
    },
  },
  SHOULDER_PADS: {
    name: 'Shoulder Pads',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 2000,
    def: 4,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Kaku'],
      treasure: ["Lepant's House"],
      loot: ['Bandit (yellow)'],
    },
  },
  BLUE_RIBBON: {
    name: 'Blue Ribbon',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 1150,
    def: 6,
    wear: [WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Kouan'],
      treasure: ["Dwarves' Vault"],
      loot: ['Beast Commander'],
    },
  },
  NECKLACE: {
    name: 'Necklace',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 1200,
    def: 7,
    wear: [WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Elves Village'],
      treasure: [],
      loot: ['Kobold (bow)'],
    },
  },
  EMBLEM: {
    name: 'Emblem',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 2700,
    def: 8,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.NOBILITY],
    effects: ['MAG +10'],
    sources: {
      shop: ['Kouan'],
      treasure: [],
      loot: ['Veteran Soldier (bow)'],
    },
  },
  FUR_CAPE: {
    name: 'Fur Cape',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 2800,
    def: 8,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Dwarf Village'],
      treasure: [],
      loot: ['Death Boar'],
    },
  },
  STAR_EARRINGS: {
    name: 'Star Earrings',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 12000,
    def: 8,
    wear: [WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: ['Auto-heal +5'],
    sources: {
      shop: [],
      treasure: [],
      loot: ['Hell Unicorn'],
    },
  },
  FEATHER: {
    name: 'Feather',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 2000,
    def: 9,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Elves Village'],
      treasure: ["Dwarves' Trail"],
      loot: ['Roc'],
    },
  },
  TOE_SHOES: {
    name: 'Toe Shoes',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 2800,
    def: 9,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Antei'],
      treasure: ['Toran Basement'],
      loot: ['Holly Spirit'],
    },
  },
  SILVER_RING: {
    name: 'Silver Ring',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 5500,
    def: 11,
    wear: [WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Kirov'],
      treasure: [],
      loot: ['Strong Arm'],
    },
  },
  SILVERLET: {
    name: 'Silverlet',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 7000,
    def: 11,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Kirov'],
      treasure: ['Qlon Cave'],
      loot: ['Shadow'],
    },
  },
  MANGOSH: {
    name: 'Mangosh',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 21000,
    def: 12,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: ['SKL +20', 'Counter-rate up'],
    sources: {
      shop: [],
      treasure: [],
      loot: ['Devil Armor'],
    },
  },
  ROSE_BROOCH: {
    name: 'Rose Brooch',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 7000,
    def: 13,
    wear: [WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: ['Antei'],
      treasure: [],
      loot: ['Shadow', 'Shadow Man'],
    },
  },
  CAPE_OF_DARKNESS: {
    name: 'Cape of Darkness',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 8500,
    def: 13,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ["Warrior's Village", 'Gregminster #2'],
      treasure: ["Neclord's Castle"],
      loot: ['Ninja'],
    },
  },
  SPEED_RING: {
    name: 'Speed Ring',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 13000,
    def: 13,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: ['SPD +15'],
    sources: {
      shop: [],
      treasure: [],
      loot: ['Mirage'],
    },
  },
  WING_BOOTS: {
    name: 'Wing Boots',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 10200,
    def: 14,
    wear: [WEAR_CLASSES.FEMALE],
    effects: ['SPD +10'],
    sources: {
      shop: ['Gregminster #1'],
      treasure: [],
      loot: ['Ninja'],
    },
  },
  POWER_RING: {
    name: 'Power Ring',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 14000,
    def: 14,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: ['PWR +20'],
    sources: {
      shop: [],
      treasure: ['Gregminster Palace'],
      loot: ['Colossus'],
    },
  },
  GUARD_RING: {
    name: 'Guard Ring',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 8500,
    def: 15,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Antei', 'Gregminster #2'],
      treasure: ['Shasarazade'],
      loot: ['Grave Master'],
    },
  },
  POWER_GLOVES: {
    name: 'Power Gloves',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 20000,
    def: 16,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: ['PWR +15'],
    sources: {
      shop: [],
      treasure: ["Dragon's Den"],
      loot: ['Wyvern'],
    },
  },
  SILVER_NECKLACE: {
    name: 'Silver Necklace',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 6000,
    def: 17,
    wear: [WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: ['Kirov'],
      treasure: ['Soniere'],
      loot: ['Demon Hound'],
    },
  },
  GOLDLET: {
    name: 'Goldlet',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 19000,
    def: 18,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY],
    effects: [],
    sources: {
      shop: [],
      treasure: ["Dragon's Den"],
      loot: ['Gigantes'],
    },
  },
  EARTH_BOOTS: {
    name: 'Earth Boots',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 22000,
    def: 18,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: [],
      treasure: ["Neclord's Castle"],
      loot: ['Phantom'],
    },
  },
  CRIMSON_CAPE: {
    name: 'Crimson Cape',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 32000,
    def: 21,
    wear: [WEAR_CLASSES.MALE, WEAR_CLASSES.FEMALE, WEAR_CLASSES.NOBILITY, WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: [],
      treasure: ['Qlon Cave'],
      loot: ['Ninja Master'],
    },
  },
  GOLD_NECKLACE: {
    name: 'Gold Necklace',
    type: ARMOR_TYPES.ACCESSORY,
    bits: 17000,
    def: 25,
    wear: [WEAR_CLASSES.KOBOLD],
    effects: [],
    sources: {
      shop: [],
      treasure: ['Seek Valley'],
      loot: ['Kerberos'],
    },
  },
};
