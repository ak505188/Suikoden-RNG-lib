...
export default class RNG {
  ...
  /** @param {number} rng */
  constructor(rng = generateRandomSeed()) {
    this.rng         = rng;
  ...
  getNext(iterations = 1) {
    ...
    for (let i = 0; i < iterations; i++) {
      rng = (Math.imul(rng, 0x41c64e6d) + 0x3039) >>> 0
    }
    return { rng, rng2: RNG.calcRNG2(rng) };
  }
...
export function determineAssassinMove(rng) {
  ...
  do {
    rng.next();
  } while (rng.getRNG2() % 100 < 51);

  // Determine Move
  rng.next();

  let r3 = rng.getRNG2() << 1;
  r3 = r3 + rng.getRNG2();
  r3 = r3 << 3;
  r3 = r3 + rng.getRNG2();
  r3 = r3 << 2;
  ...
  return { move, move_name, rng: rng.getRNG() };
}
