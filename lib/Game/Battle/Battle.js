...
export default class Battle {
  ...
  dispatch(queue) {
    ...
    for (const i in queue) {
      const { action, actor, result, target } = queue[i];
      if (action === ACTION_TYPES.ATTACK) {
        attackQueue.push({ actor, target, result });
      }
    }
    /** @type {string[]} */
    const attackResults = attackQueue.map(({ actor, target, result }) => {
      console.log(`${actor.name} attacks ${target.name}, result: ${result}`);
      target.wait = true;
