...
export function isTripleWin(cursor, rng2, speed = 0, rngModifier = 0) {
  let r18 = cursor.getValue();

  let r2 = Math.floor(speed / 8);
  let r3 = r18;
  let r4 = Math.abs(r18 >> 0x1f);
  r3 += r4;
  r3 = r3 >> 1;
  r3 = (r3 + 0xfffb) & 0xffff;
  if ((r3 & 0x8000) > 0) {
...
export function isDoubleWin(cursor, rng2, speed = 0, rngModifier = 0) {
  ...
  let r2 = 5 + Math.floor(speed / 4) - r18
...
export function simulateRoll(cursor, rng, speed = 0, rngModifier = 0) {
  rng.next();
  if (isTripleWin(cursor, rng.getRNG2(), speed, rngModifier)) {
    rng.next();
    const roll = getStandardRollDie(rng, 0, false);
    return `${roll}${roll}${roll}`
  }
  rng.next();
  if (isTripleLose(cursor, rng.getRNG2(), speed, rngModifier)) {
    return '111';
  }
  rng.next();
  if (isDoubleWin(cursor, rng.getRNG2(), speed, rngModifier)) {
    return '456';
  }
  rng.next();
  if (isDoubleLose(cursor, rng.getRNG2(), speed, rngModifier)) {
    return '123';
  }
  rng.next();
  if (isPiss(cursor, rng.getRNG2(), speed, rngModifier)) {
    // Rolls are still calculated, so need to simulate
    rng.next();
    getStandardRollFull(rng, rngModifier);
    return 'OUT';
  }
  rng.next()
