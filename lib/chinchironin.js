...
export function simulateRoll(cursor, rng, speed = 0, rng_modifier = 0) {
  ...
  if (isTripleWin(cursor, rng.getRNG2(), speed, rng_modifier)) {
    ...
    return { roll: `${roll}${roll}${roll}`, rng: rng.getRNG(), index: rng.count };
  }
  rng.next();
  if (isTripleLose(cursor, rng.getRNG2(), speed, rng_modifier)) {
    return { roll: '111', rng: rng.getRNG(), index: rng.count };
  }
  rng.next();
  if (isDoubleWin(cursor, rng.getRNG2(), speed, rng_modifier)) {
    return { roll: '456', rng: rng.getRNG(), index: rng.count };
  }
  rng.next();
  if (isDoubleLose(cursor, rng.getRNG2(), speed, rng_modifier)) {
    return { roll: '123', rng: rng.getRNG(), index: rng.count };
  }
  rng.next();
  if (isPiss(cursor, rng.getRNG2(), speed, rng_modifier)) {
    ...
    return { roll: 'OUT', rng: rng.getRNG(), index: rng.count };
  }
  rng.next()
  const roll = getStandardRollFull(rng, rng_modifier);
  return { roll, rng: rng.getRNG(), index: rng.count };
}
...
export function simulateOpponentRoll(rng, cursor, speed, is_tai_ho, rng_modifier = 0) {
  while (true) {
    ...
    if (isValidTaiHoRoll(roll.roll)) return roll;
  }
}
