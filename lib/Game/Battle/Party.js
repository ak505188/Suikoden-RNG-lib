...
export class Party {
  ...
  constructor(combatants) {
    ...
    this.combatants.forEach((combatant, index) => combatant.slot = index + 1);
  ...
  getFrontRow() {
    return this.getLivingCombatants().slice(3);
  }
...
export class EnemyParty extends Party {
  ...
  wakePartyUp(rng) {
    this.combatants
      ...
      .forEach(combatant => combatant.status[STATUS.SLEEP] = rng.next().getRNG2() * 100 / 0x7fff >= 50)
  }
}
