...
function cDiv(num, den) {
  const magnitude = Math.floor(Math.abs(num) / Math.abs(den));
  return (num < 0) === (den < 0) ? magnitude : -magnitude;
}

// JS's `|` (and other bitwise ops) already coerce to a signed 32-bit integer, so this is a
// direct equivalent of the source's mask-then-resign dance.
function signed32(v) {
  return v | 0;
}
