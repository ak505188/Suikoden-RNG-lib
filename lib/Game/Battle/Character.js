...
import { ACTION_TYPES, DEFAULT_ACTION } from './Actions.js';
...
export default class Character extends Combatant {
  ...
  handleAttack(target, rng) {
    ...
    if (!target.speciesFlags.canDodge) willHit = true;
    if (target.speciesFlags.forceMissOnHit) willHit = false;
    ...
    if (willHit) {
      ...
      let baseDmg = rng.next().calculateDamageRoll(this.ATK, target.ARM);
      // TODO: Element check. If weak, base += base/2
      // There is a supposed mastery byte, I don't think it's real.
      baseDmg = Math.max(baseDmg, 1);
      const multiplier = willCrit ? 3 : 1;
      return baseDmg * multiplier;
    }

    // Handle miss branch
    let willCounter = false;
    if (target.speciesFlags.canCounter && this.range !== RANGE.LONG) willCounter = true;
  }
