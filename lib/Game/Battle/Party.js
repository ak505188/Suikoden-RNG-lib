...
export class Party {
  ...
  getFrontRow() {
    return this.getLivingCombatants().filter(c => c.position < 4 && !c.outOfFight);
  }

  clearBusy() {
    this.combatants.forEach(combatant => combatant.busyUntil = 0);
  }
   ...
   */
  isValidTarget(targetIndex, targetType, tick) {
    ...
    if (targetType === TARGET.ANY) {
      ...
      if (target.isBusy(tick)) return ACTION_VALIDITY.BUSY;
