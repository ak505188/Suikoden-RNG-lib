...
export default class Battle {
  ...
  resetTurn() {
    ...
    /** @type {Map<number, (() => void)[]>} tick -> callbacks, run in queue order */
    this.events = new Map();
  }

  /**
   * Schedules fn for step 2 of that tick (after the turn step, before the damage commit),
   * e.g. a basic attack's calc_damage rand() at t0 + timing.damage.
   * @param {number} tick
   * @param {() => void} fn
   */
  queueEvent(tick, fn) {
 ...
 if (possibleActors[i].spdRoll > possibleActors[maxIndex].spdRoll) maxIndex = i;
  ...
  // Mirrors the round driver LAB_800f72f0. The animation pass runs after all three;
  // busyUntil models its effect.
  tick() {
