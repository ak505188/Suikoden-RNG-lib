...
import { ACTION_VALIDITY, ACTION_TYPES, ATTACK_RESULTS, DEFAULT_ACTION } from './Actions.js';
...
export default class Character extends Combatant {
  ...
  handleAttack(target, rng) {
    ...
    return this.willGetCountered(target) ? ATTACK_RESULTS.COUNTERED : ATTACK_RESULTS.MISSED;
  }

  isValidAction({ party, enemies }) {
    switch(this.action.type) {
      case ACTION_TYPES.ATTACK: {
        // TODO: Validate target
        const targetIndex = this.action.target || 0;
        const targetEnemy = enemies[targetIndex];
        const result = this.handleAttack(targetEnemy, rng);
        return { action: ACTION_TYPES.ATTACK, targetIndex, result };
      }
      case ACTION_TYPES.RUNE: {
        // TODO: Handle Rune Unites
        if (this.rune.type === RUNE_TYPES.MAGIC) {
  ...
  resolveAction({ party, enemies, rng }) {
