...
export function isTripleWin(cursor, rng_short, rngModifier = 0) {
  let r2 = 0;
  let r3 = cursor.value;
  ...
  let r5 = (rng_short + rngModifier) % 100;
  process.env.SHOULD_LOG && console.log(`3W r2:${r2} r5:${r5}`);
...
export function isTripleLose(cursor, rng_short, rng_modifier = 0) {
  ...
  process.env.SHOULD_LOG && console.log(`3L r2:${r2} r5:${r5}`);
  if (r2 <= 0) {
    return false;
  }
...
export function isDoubleWin(cursor, rng_short, rng_modifier = 0) {
  ...
  process.env.SHOULD_LOG && console.log(`2W r2:${r2} r5:${r5}`);
...
export function isDoubleLose(cursor, rng_short, rng_modifier = 0) {
  ...
  process.env.SHOULD_LOG && console.log(`2L r2:${r2} r5:${r5}`);
...
export function getStandardRollDie(rng, rng_modifier = 0, can_be_one = true, should_log = false) {
  let counter = 0;
  const changeCounter = count => counter = count;
  let roll = null;
  while (roll === null) {
